...
const luts = [
  {
    name: "HN Signature Cinematic LUTs",
    price: "$100",
    img: "https://images.unsplash.com/photo-1500051638674-ff996a0ec29e?w=800&q=80",
    featured: false,
  },
  {
    name: "Films LUTs V1",
...
export function LutsSection() {
  return (
    <section id="luts" className="bg-paper px-4 py-8 md:px-6 md:py-10">
      <div className="w-full rounded-3xl border-2 border-ink/20 p-6 md:p-10">
        <div className="grid gap-6 md:grid-cols-[1fr_2fr] md:items-end">
          ...
          <p className="text-sm text-muted-foreground md:text-base">
            Turns out, a beautiful & strategic website doesn't have to start from scratch. Our
            customers prove that with the right template, you can launch a custom-looking site.
          </p>
        </div>
            ...
            <article key={l.name + l.price} className="group relative">
              ...
              >
                <div className="flex items-start justify-between gap-3 px-2 pb-4 pt-2 pr-16">
                  ...
                  <span
                    className={`font-mono text-sm font-normal shrink-0 ${
                      l.featured ? "text-ink" : "text-paper"
                    }`}
                  >
                    {l.price}
        ...
        <div className="mt-12 text-center">
          <p className="font-display text-3xl font-black uppercase leading-tight md:text-4xl">
            Keep{" "}
            <span className="text-primary [text-shadow:_-1px_-1px_0_var(--ink),1px_-1px_0_var(--ink),-1px_1px_0_var(--ink),1px_1px_0_var(--ink)]">
              creating
            </span>{" "}
            until you <br />
            find your{" "}
